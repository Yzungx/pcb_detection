...
#----------------------------------------------
template_image = cv2.imread("./image/display_image/anh_mau_2_resize.jpg")
#----------------------------
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.status_calib = False

        data = load('./calib/calib_data/Calib_data.npz')
        self.cam_id  = int(data['cam'])
        self.camMatrix = data['camMatrix']
        self.distCof = data['distCoef']
        self.rVector = data['rVector']
        self.tVector =  data['tVector']
    ...
    def calib(self):
        ...
        self.status_calib = True
        os.system('python Calib_image.py')
    ...
    def save_calib(self):
        if self.status_cam == True :
            os.makedirs('./calib/images',exist_ok=True)
            cv2.imwrite('./calib/images/'+str(self.calib_dem)+'.jpg',cv_img)
            time.sleep(0.5)
            
            img_calib = cv2.imread('./calib/images/'+str(self.calib_dem)+'.jpg')
    ...
    def cam_id_connect(self):
        ...
        np.savez("./calib/calib_data/Calib_data",
                     camMatrix=self.camMatrix,
                     distCoef=self.distCof,
                     rVector=self.rVector,
    ...
    def show_loi(self):
        ...
        if i > 0:
            ...
            for j in range(i):
                ...
                # if labels[j] == 'chip_below_right' or labels[j] == 'ic_above_right':
                #     print('')
                # else:
                ...
                if labels[j] == 'scratch':
                    ...
                    self.show_errorImg(img_crop,labels[j])
                # elif labels[j] == 'tu_nguoc' or labels[j] == 'chip_below_left' or labels[j] == 'chip_above_left' or labels[j] == 'chip_above_right' or labels[j] == 'ic_below_left':                 
                #     self.dem_nguoc = self.dem_nguoc + 1
                # elif labels[j] == 'hong':
                #     self.dem_thieu = self.dem_thieu + 1
        self.dem_loi(self.dem_xuoc)
        # self.dem_loi(self.dem_xuoc,self.dem_nguoc,self.dem_thieu)
    #---------------------------------------------------
    def dem_loi(self, dem_xuoc):
    # def dem_loi(self,dem_xuoc,dem_nguoc,dem_thieu):
        if dem_xuoc :
            if dem_xuoc == 0:
                self.uic.message.append("  The PCB hasn''t any scratches ")
            elif dem_xuoc == 1:
                self.uic.message.append("  The PCB has "+str(dem_xuoc)+" scratch ")
            else:
                self.uic.message.append("  The PCB has "+str(dem_xuoc)+" scratches ")       
        # if dem_nguoc :
    ...
    def show_message(self,i):
        if i == 0:
            self.uic.message.append("  The PCB hasn't any errors")
            Arduino_serial.write('t'.encode())
        if i > 0:
            self.uic.message.append("  The PCB has errors")
    ...
    def read_serial(self):
        if com_status == True:
            ...
            while (self.cap.isOpened()):
                ...
                if self.status_calib == False:
                    print('')
                else:
                    '''-----------------'''
                    h, w = cv_img.shape[:2]
                    newcameramtx, roi = cv2.getOptimalNewCameraMatrix (self.camMatrix, self.distCof, (w,h), 1, (w,h))
                    dst = cv2.undistort (cv_img, self.camMatrix, self.distCof, None , newcameramtx)
                    x, y, w, h = roi
                    dst = dst[y:y+h, x:x+w]
                    cv_img = dst
...
opt  = {
	...
	"weights": "./cfg/last600_600.pt", # Path to weights file default weights are for nano model
	"yaml"   : "./cfg/mydataset1.yaml",
...
def yolo (img0):
	...
	# Apply NMS
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if conf > 0.6:
                        ...
                        # elif label == 'hong':
                        #     cv2.rectangle(img0, (x1,y1), (x2,y2), (0,   255,255), 4)
                        #     labels.append(label)
                        #     boxes.append([x1, y1, x2, y2])
                        #     i=i+1

                        # # if label == 'tu_nguoc' or label == 'chip_below_left' or label == 'chip_above_left' or label == 'chip_above_right' or label == 'ic_below_left':
                        # # if label == 'tu_nguoc' or label == 'chip_below_left' or label == 'chip_above_left' or label == 'chip_above_right' or label == 'ic_below_left' or label == 'ic_below_right' or label == 'ic_above_left':
                        # elif label == 'chip_above_right':
                        #     cv2.rectangle(img0, (x1,y1), (x2,y2), (0,0,255), 4)
                        #     labels.append(label)
                        #     boxes.append([x1, y1, x2, y2])
                        #     i=i+1
                        #     # plot_one_box(xyxy, img0, label=str(conf), color=colors[int(cls)], line_thickness=3)
                        # elif label == 'chip_below_left':
                        #     cv2.rectangle(img0, (x1,y1), (x2,y2), (0,0,255), 4)
                        #     labels.append(label)
                        #     boxes.append([x1, y1, x2, y2])
                        #     i=i+1
                        # elif label == 'ic_below_left':
                        #     cv2.rectangle(img0, (x1,y1), (x2,y2), (0,0,255), 4)
                        #     labels.append(label)
                        #     boxes.append([x1, y1, x2, y2])
                        #     i=i+1
        ...
        #             elif label == 'chip_below_right' or label == 'ic_above_right':
        #                 if conf>0.5:
        #                     cv2.rectangle(img0, (x1,y1), (x2,y2), (0,0,255), 4)
        #                 print('')
...
class capture_video(QThread):
    ...
    def run(self):
        ...
        #img_yolo = cv2.imread('./image/anh_yolo.png')
