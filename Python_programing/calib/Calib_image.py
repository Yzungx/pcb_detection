...
#--------------------------------------------------------------
CHECK_DIR = os.path.isdir("./ChessboardCorners/")

if not CHECK_DIR:
    os.makedirs("./ChessboardCorners/")
...
for file in files:
    ...
    if ret == True:
        ...
        cv.imwrite ( "./ChessboardCorners/"+str(file) , img)
...
#--------------------------------------------------
img_test = cv.imread ( 'image12.png' )
h, w = img.shape[:2]
newcameramtx, roi = cv.getOptimalNewCameraMatrix (camMatrix, distCof, (w,h), 1, (w,h))
# giải nén
dst = cv.undistort (img_test, camMatrix, distCof, None , newcameramtx)
# cắt ảnh
x, y, w, h = roi
dst = dst[y:y+h, x:x+w]
